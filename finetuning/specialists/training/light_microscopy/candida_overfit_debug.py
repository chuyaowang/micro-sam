...
def _safe_load_checkpoint(checkpoint_path):
    ...
    ``AttributeError: Can't get attribute '<Name>' on <module '__main__'>``. We only consume
    ``model_state`` (tensors), so any global that cannot be imported is replaced with a harmless
    placeholder. Those placeholders live only inside the ``init`` blob, which ``_strip_ddp_prefix``
    drops before re-saving, so they are never used or persisted.
    """
    import pickle
    import types
...
def _strip_ddp_prefix(checkpoint_path, figure_dir):
    ...
    Always writes a cleaned copy into ``figure_dir`` and returns its path. Two things are cleaned:
    the ``module.`` prefix is stripped from ``model_state`` (a no-op for single-GPU checkpoints), and
    the trainer ``init`` blob is dropped. Dropping ``init`` is what makes the copy portable -- it
    pickles the datasets and their transforms, often under ``__main__`` (see ``_safe_load_checkpoint``),
    so keeping it would re-introduce the cross-process unpickling error on any later load. Only
    ``model_state`` is needed downstream.
    """
    ...
    print(f"[overfit-comparison] wrote portable checkpoint (DDP prefix stripped, init dropped) -> {normalized_path}")
